...
ifneq ($(words $(LOCAL_LINK_MODE)),1)
  $(call __ndk_info,ERROR:$(LOCAL_MAKEFILE):$(LOCAL_MODULE): LOCAL_LINK_MODE must be either 'c' or 'c++'.)
  $(call __ndk_error, Aborting)
endif
ifeq ($(filter c c++ ,$(LOCAL_LINK_MODE)),)
  $(call __ndk_info,ERROR:$(LOCAL_MAKEFILE):$(LOCAL_MODULE): LOCAL_LINK_MODE must be either 'c' or 'c++'.)
  $(call __ndk_error, Aborting)
endif
endif
...
# we are want to build release and install target when executable $sys-buid $LOCAL_MODULE
$(LOCAL_MODULE): $(call module-get-installed,$(LOCAL_MODULE))
...
#add target detected
$(call add-target-module,$(LOCAL_MODULE),$(LOCAL_MODULE) $(cleantarget))

$(cleantarget): PRIVATE_MODULE      := $(LOCAL_MODULE)
$(cleantarget): PRIVATE_TEXT        := [$(TARGET_ARCH)]
ifneq ($(LOCAL_BUILT_MODULE_NOT_COPIED),true)
#The command will be executed repeatedly and 
#if the module is static library ,but that's okay
$(cleantarget): PRIVATE_CLEAN_FILES := $(LOCAL_BUILT_MODULE) $(LOCAL_INSTALLED)\
                                       $(addsuffix $($(my)OBJS)/$(LOCAL_MODULE),$(dir $(LOCAL_BUILT_MODULE)))
else
#Maby only prebuilt_static_library.
#it's looke like 'rm -rf $(TARGET_OUT)/objs/$(LOCAL_MODULE)'
$(cleantarget): PRIVATE_CLEAN_FILES := $(addsuffix /$($(my)OBJS)/$(LOCAL_MODULE),$(TARGET_OUT)) 
endif

# we are not to want to remove Applicaton.mk 
#$(cleantarget): PRIVATE_CLEAN_FILES += $(wildcard $(LOCAL_PATH)/Application.mk)

#Allow a more $(cleantarget) target
$(cleantarget)::
...
# always define XXXX when building binaries
#
#LOCAL_CFLAGS := -DXXXX $(LOCAL_CFLAGS)
...
# Don't to need the flags for To build THRID_PARTY_EXEC and KERNEL_BINARY and  BOOTLOADER 
...
#
ifeq (,$(filter THRID_PARTY_EXEC  KERNEL_BINARY BOOTLOADER,$(call module-get-class,$(LOCAL_MODULE))))
...
endif #ifeq (,$(filter THRID_PARTY_EXEC  KE ...
#
...
#if user set -DNDEBUG flags then define NDEBUG macro.
LOCAL_CFLAGS := -g -O0 -UNDEBUG -Wall $(LOCAL_CFLAGS)
else
#if user set -UNDEBUG flags then undef NDEBUG macro.
LOCAL_CFLAGS :=  -O2 -DNDEBUG -Wall $(LOCAL_CFLAGS)
endif
endif
...
# Add LOCAL_DEPS_MODULES for every source file.
LOCAL_SRC_FILES := $(strip $(LOCAL_SRC_FILES))
$(foreach _src,$(LOCAL_SRC_FILES),\
	$(eval $(call local-prebuilt-path,$(_src)): $(LOCAL_DEPS_MODULES))) \
...
#default values c mode.
ifeq ($(LOCAL_LINK_MODE),c++)
$(LOCAL_BUILT_MODULE): PRIVATE_LD_EXE := $(PRIVATE_CXX)
else
$(LOCAL_BUILT_MODULE): PRIVATE_LD_EXE := $(PRIVATE_CC)
endif
...
#Not TEST modules.
ifeq (,$(filter  TEST,$(call module-get-class,$(LOCAL_MODULE))))
include $(BUILD_SYSTEM)/build-th3-binary-obj.mk
endif

ifneq (,$(filter SHARED_LIBRARY EXECUTABLE TEST,$(call module-get-class,$(LOCAL_MODULE))))
...
ifneq (,$(filter  TEST,$(call module-get-class,$(LOCAL_MODULE))))
 include $(BUILD_SYSTEM)/build-th3-binary-obj.mk
endif
#
...
#Already add where in file 'prebuilt-library.mk'
#ALL_PREBUIT_LIBRARIES += $(LOCAL_BUILT_MODULE)
endif

$(call add-target-module,$(LOCAL_MODULE),$(LOCAL_BUILT_MODULE))
#
...
$(call add-target-module,$(LOCAL_MODULE),$(LOCAL_INSTALLED))
