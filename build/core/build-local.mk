...
ifeq ($(NDK_LOG),1)
    $(info SYS-BUILD: NDK installation path auto-detected: '$(NDK_ROOT)')
endif
ifneq ($(words $(NDK_ROOT)),1)
    $(info SYS-BUILD: You NDK installation path contains spaces.)
    $(info SYS-BUILD: Please re-install to a different location to fix the issue !)
    $(error Aborting.)
endif
# Reference may be elsewhere
SYSBUILD_ROOT := $(NDK_ROOT)
...
# path by looking at the make.mk file in the current directory or
# any of its parents.
#
...
SYSBUILD_PROJECT_PATH := $(strip $(SYSBUILD_PROJECT_PATH))
...
# will define SYSBUILD_PROJECT_PATH to simply '.'
#
...
ifndef SYSBUILD_PROJECT_PATH
    ...
    $(call __ndk_info,Please define the SYSBUILD_PROJECT_PATH variable to point to it.)
    $(call __ndk_error,Aborting)
endif

# Check that there are no spaces in the project path, or bad things will happen
ifneq ($(words $(SYSBUILD_PROJECT_PATH)),1)
    $(call __ndk_info,Your Android application project path contains spaces: '$(NDK_PROJECT_PATH)')
    $(call __ndk_info,The SYS-BUILD build cannot work here. Please move your project to a different location.)
    $(call __ndk_error,Aborting.)
endif

NDK_PROJECT_PATH:= $(SYSBUILD_PROJECT_PATH)
...
NDK_APP_OUTPUT_DIR := $(APP_OUTPUT_DIR)
ifdef NDK_APP_OUTPUT_DIR
NDK_APP_OUT := $(NDK_APP_OUTPUT_DIR)/obj
NDK_APP_LIBS_OUT := $(NDK_APP_OUTPUT_DIR)/libs
$(call ndk_log,Change path to `$(NDK_APP_OUTPUT_DIR)`)
endif
$(call ndk_log,Ouput path: $(NDK_APP_OUT))
$(call ndk_log,Install path: $(NDK_APP_LIBS_OUT))
