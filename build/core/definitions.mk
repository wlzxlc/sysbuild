...
#
# Common definitions for the SYS-BUILD build system
#
...
# -----------------------------------------------------------------------------
local-makefile = $(lastword $(filter %Android.mk %make.mk,$(MAKEFILE_LIST)))
...
#if the path contain '~' character
__ndk_dir := $(subst ~,$(HOME),$1)
...
$$(call add-target-module,__ndk_app_fake_generate_dir,$$(__ndk_dir))
$1:
	@$$(call host-mkdir,$$@)
endif
endef
...
generate-file-dir = \
$(if $(filter 1,$(words $1)),\
$(eval $(call ev-generate-file-dir,$1)),\
$(call __ndk_info,ERROR: The parameter must be a path in the function 'generate-file-dir'.)\
$(call __ndk_info,ERROR: ($1))\
...
module-add = \
  ...
  $(if $(call module-class-is-installable,$(LOCAL_MODULE_CLASS)),\
    $(eval LOCAL_INSTALLED := $(addsuffix /$(notdir $(LOCAL_BUILT_MODULE)),\
	$(call get-relate-path,$(LOCAL_RELATE_MODE),libs))),\
	$(eval LOCAL_INSTALLED := $(LOCAL_BUILT_MODULE))\
  )\
  $(eval LOCAL_RELEASE_PATH := $(subst ;, ,$(LOCAL_RELEASE_PATH)))\
  $(eval LOCAL_RELEASE_PATH := $(subst ~,$(HOME),$(LOCAL_RELEASE_PATH)))\
  $(eval LOCAL_RELEASE_PATH := $(sort $(strip $(patsubst %/,%,$(LOCAL_RELEASE_PATH)))))\
  $(eval LOCAL_RELEASE_PATH := $(sort $(strip $(abspath $(LOCAL_RELEASE_PATH)))))\
  $(if $(LOCAL_RELEASE_PATH),$(eval LOCAL_INSTALLED_RELEASE := \
  $(addsuffix /$(notdir $(LOCAL_INSTALLED)),$(LOCAL_RELEASE_PATH))))\
...
module-get-installed = $(if $(__ndk_modules.$1.INSTALLED_RELEASE),\
 $(__ndk_modules.$1.INSTALLED_RELEASE),$(__ndk_modules.$1.INSTALLED))

module-is-installed-release = $(if $(__ndk_modules.$1.INSTALLED_RELEASE),$(true),$(false))
...
check-defined-kernel-LOCAL_KERNEL_PATH = $(if $(strip $(wildcard $(LOCAL_KERNEL_PATH)/Makefile)),,\
  $(call assert-defined,LOCAL_KERNEL_PATH) \
  $(call __ndk_info,$(LOCAL_MAKEFILE):$(LOCAL_MODULE): `$(LOCAL_KERNEL_PATH)` Not a valid path of the kernel.) \
  $(call __ndk_error,Please correct error. Aborting)\
)
...
handle-module-built = \
$(if $(strip $(LOCAL_RELATE_MODE)),,$(eval LOCAL_RELATE_MODE := board)) \
    $(eval LOCAL_RELATE_MODE := $(strip $(LOCAL_RELATE_MODE))) \
    $(call relate-mode-checks,$(LOCAL_RELATE_MODE)) \
    $(eval LOCAL_BUILT_MODULE := $(addsuffix /$(LOCAL_MODULE_FILENAME),$(call get-relate-path,$(LOCAL_RELATE_MODE),obj)))\
    $(eval LOCAL_OBJS_DIR     := $(addsuffix $(TARGET_OBJS)/$(LOCAL_MODULE),$(dir $(LOCAL_BUILT_MODULE))))
...
# -----------------------------------------------------------------------------
all-makefiles-under = $(wildcard $1/*/make.mk)
...
# -----------------------------------------------------------------------------
all-subdir-makefiles = $(call all-makefiles-under,$(LOCAL_PATH))
...
NDK_APP_VARS_REQUIRED := APP_PLATFORM APP_ARCH APP_BOARD APP_BUILD_SCRIPT APP_OUTPUT_DIR
...
NDK_APP_VARS_OPTIONAL := APP_OPTIM APP_CPPFLAGS APP_CFLAGS APP_CXXFLAGS APP_LDFLAGS \
                         APP_ABI APP_MODULES APP_PROJECT_PATH APP_STL APP_SHORT_COMMANDS \
                         APP_PIE APP_THIN_ARCHIVE APP_VENDOR APP_WORKSPACE  APP_SDK_DIR \
						 APP_LSP_DIR APP_RELEASE_DIR APP_TOOLCHAIN_SYSROOT APP_ALIAS_BOARD
...
import-add-path = \
  $(if $(strip $(wildcard $1)),\
    $(call ndk_log,Adding import directory<import-add-path>: $1)\
    $(eval __ndk_import_dirs += $1)\
  ,\
...
import-find-module = $(strip \
      ...
      $(foreach __import_dir,$(__ndk_import_dirs),\
        $(if $(__imported_module),,\
          $(call ndk_log,  Probing $(__import_dir)/$1/make.mk)\
          $(if $(strip $(wildcard $(__import_dir)/$1/make.mk)),\
...
import-module = \
    ...
    ,\
      ...
      $(if $(__imported_path),\
        ...
        $(eval include $(__imported_path)/make.mk)\
        $(eval __ndk_import_depth := $(__ndk_import_depth:%x=%))\
      ,\
...
# kernel image
# <foo> -> <foo> (we assume it is already well-named)
$(call module-class-register,KERNEL_IMG,,)
...
#Register the 'gtest' ,path module_path/gtest ,StaticLibrary name libgtest_main
$(call ndk-stl-register,\
    gtest,\
    gtest,\
    libgtest_main,\
)
#Register the 'trace' ,path module_path/trace ,StaticLibrary name libtrace
$(call ndk-stl-register,\
    trace,\
    trace,\
    libtrace,\
...
include $(BUILD_SYSTEM)/th3-defined.mk
