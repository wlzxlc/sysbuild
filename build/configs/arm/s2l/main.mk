...
# 以下三个路径分别指向S2L工程的顶层目录/SDK的目录/LSP的目录，强制在sys-build config时定义，尽量不要二次覆盖
...
makeos:
	...
	$(Q) cp -af $(TARGET_SDK_DIR)/out/$(TARGET_BOARD)/kernel/zImage \
		$(TARGET_RELEASE_DIR)/boards/$(TARGET_PLATFORM)/$(TARGET_BOARD)/zImage
	$(Q) export LINUX_TOOLCHAIN_PREFIX=$(shell dirname $(APP_ARM_TOOLCHAIN))/../..; \
		export ELF_TOOLCHAIN_PREFIX=$(shell dirname $(APP_ARM_TOOLCHAIN))/../..; \
		. $(TARGET_SDK_DIR)/build/env/Linaro-multilib-gcc4.8.env; \
